...
struct virtio_virtq *virtq_init(unsigned index) {
  ...
  // 7. write the physcial number of the first page of the queue to the QueuePFN register
...
void virtio_blk_init(void) {
  ...
  // read 64-bit capacity (in 512-byte sectors) from device config
  uint32_t lo = virtio_reg_read32(0x100);
  uint32_t hi = virtio_reg_read32(0x104);

  // get the disk capacity
  blk_capacity = (uint64_t)lo * SECTOR_SIZE;
